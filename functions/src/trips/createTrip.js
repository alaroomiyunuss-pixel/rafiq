...
// MVP: allowed Saudi city IDs (must match sa_cities.json)
...
const createTrip = onCall({ region: "me-central1" }, async (request) => {
  ...
  // Required fields
  if (!originCityId || !destCityId || !dateTime || !totalSeats ||
      !pricePerSeatHalalas || !vehicleId || !meetingMode) {
  ...
  if (originCityId === destCityId) {
    throw new HttpsError("invalid-argument", "مدينة الانطلاق والوصول يجب أن تكونا مختلفتين");
  }

  // DateTime: must be at least 4h in future
  ...
  if (tripDate.getTime() - Date.now() < MIN_LEAD_TIME_MS) {
    throw new HttpsError("invalid-argument", "يجب أن يكون موعد الرحلة بعد 4 ساعات على الأقل");
  }
  ...
  // Price
  if (!Number.isInteger(pricePerSeatHalalas) || pricePerSeatHalalas < MIN_PRICE_HALALAS) {
  ...
  if (meetingMode === "MANUAL_ADDRESS") {
    if (!meetingPoint || !meetingPoint.address || typeof meetingPoint.address !== "string") {
      throw new HttpsError("invalid-argument", "MANUAL_ADDRESS يتطلب عنوان نصي");
    }
  }
  // DRIVER_PICKS_UP: passenger sets point during booking, no validation here

  // Notes
  const trimmedNotes = notes ? String(notes).substring(0, MAX_NOTES_LENGTH) : "";

  // --- Load city names for denormalization ---
